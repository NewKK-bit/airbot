...
/* ---------- 외부 사이트 바로가기 버튼 ----------
   설정된 일정으로 각 항공권 사이트를 개별로 여는 버튼 묶음을 컨테이너에 렌더링 */
function renderSiteButtons(container, getQuery) {
  container.innerHTML = "";
  DeepLink.PROVIDERS.forEach((p) => {
    const b = document.createElement("button");
    b.className = "mini-btn";
    b.style.color = p.color;
    b.innerHTML = `<span class="dot" style="background:${p.color}"></span><span style="color:var(--text)">${p.label}</span>${ICONS.external}`;
    b.addEventListener("click", () => {
      const q = typeof getQuery === "function" ? getQuery() : getQuery;
      if (!q) return;
      openTab(p.build(q), false);   // 확인용이므로 앞 탭으로
    });
    container.append(b);
  });
}
window.renderSiteButtons = renderSiteButtons;
